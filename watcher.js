...
const runBuildOnEvent = (event, filePath) => {
	if (!isBuilding) {
		const mustBuildAll = event === 'add' || event === 'unlink'
		const buildPath = mustBuildAll ? 'all' : filePath
		...
		buildDev(buildPath).onFinishBuild(() => {
			buildProd(buildPath).onFinishBuild((code) => {
				if (code === 0) {
